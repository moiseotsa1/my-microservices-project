...
echo "🔔 CRÉATION DU NOTIFICATION SERVICE"
echo "=================================="

# Création des dossiers
mkdir -p microservices/notification-service/src

# Dockerfile pour notification-service
cat > microservices/notification-service/Dockerfile << 'DOCKERFILE'
FROM node:16-alpine
WORKDIR /app
COPY package*.json ./
RUN npm install
...
# package.json pour notification-service
cat > microservices/notification-service/package.json << 'PKGJSON'
{
...
cat > microservices/notification-service/src/app.js << 'APPJS'
...
echo "✅ Notification Service créé !"
